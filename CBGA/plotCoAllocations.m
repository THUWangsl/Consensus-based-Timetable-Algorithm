function plotCoAllocations(Params, allocations, agents, tasks)
    ...
    for ii = 1: Params.numAgents
        ...
        for jj = 1: length(taskPath)
            ...
            if length(coAgents) == tasks(taskID).numAgents
                ...
                plot([realStartTime, realStartTime + tasks(taskID).duration], [0.7, 0.7], 'k-', 'LineWidth', 10);
                plot([allocations(ii).timeTable(ii, taskID), realStartTime], [0.7, 0.7], 'k--', 'LineWidth', 2.0);
                text([realStartTime + tasks(taskID).duration/ 2], [1.5], ['t_{' num2str(taskID) '}']);
            end
        end
    end
